...
export default function SignUpComponent () {
    ...
    const onClickSendEmail = async (e) => {
        ...
        //email.sendMailMethod('보내는 데이터 문종!!!!!!!!!!');
        axios({
            url:'/mail',
            method:'POST',
            data: {
                yourname: "선종",
                youremail: "seulki4994@naver.com",
                yoursubject: "오늘의집 선종님 가입을 환영합니다:)♥",
                yourmessage: "Message 메시지내용입니다. .................!!!!!!!!!!!!!!!!!!!" + generateRandomNumber()
            }
        })
        .then((res)=>{

            
            console.log('노드메일러 전송 시작!');
            console.log(  res );
            const  result = JSON.parse(res.config.data);
            console.log(  result  );
            console.log(  result.yourname  );
            console.log(  result.youremail  );
            console.log(  result.yoursubject  );
            console.log(  result.yourmessage  );
        })
        .catch((err)=>{
            console.log( err );
        });
