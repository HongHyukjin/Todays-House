...
export default function Sub1ComponentChild({ 사진 }) {
    ...
    const onClickZzim =(e,id,이미지)=>{
        ...
        let arr = [];
        if(localStorage.getItem('zzim') !== null){
            arr = JSON.parse(localStorage.getItem('zzim'));
            arr = [value, ...arr];
            localStorage.setItem('zzim', JSON.stringify(arr));
        }
        else{
            arr = [value];
            localStorage.setItem('zzim', JSON.stringify(arr));
                        ...
                        <div className="pitbox" key={id}>
                            ...
                            <div className="bodybox">
                                ...
                                <div className="body-button">
                                    <button className="heart" onClick={(e)=>onClickZzim(e,id,item.이미지2)}><svg class="icon" aria-label="좋아요" width="24" height="24" fill="transparent" stroke="#424242" stroke-width="1.1" viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet"><path d="M23.22 7.95c.4 4.94-2.92 9.71-10.92 13.85a.47.47 0 0 1-.42 0C3.88 17.66.56 12.9.96 7.93 1.54 2.48 8.28.3 12.1 4.7c3.8-4.4 10.55-2.22 11.13 3.25z"></path></svg></button>
