...
const onClickDelete = (e) =>{
    e.preventDefault();
    const formData={
        "idx":Number(state.집들이.idx)
    }
    $.ajax({
        url:'http://localhost:8080/JSP/ohouse/house_delete_action.jsp',
        type:'post',
        data:formData,
        success(res){
            console.log('AJAX 성공');
            console.log(res);
            alert('삭제되었습니다');
            window.location.href='#/마이페이지/집들이';
        },
        error(err){
                    ...
                    <div className="info-box">
                        <dl className='up'>
                            ...
                            <div className="info">
                                ...
                                <dd>{state.집들이.family}</dd>
                            </div>

                        </dl>
                        <dl className='bottom'>
                            <div><dt>공간</dt><dd>{state.집들이.place}</dd></div>
                            <div><dt>평수</dt><dd>{state.집들이.pyeong}</dd></div>
                            <div><dt>작업</dt><dd>{state.집들이.worker}</dd></div>
                            <div><dt>분야</dt><dd>{state.집들이.category}</dd></div>
                            <div><dt>가족형태</dt><dd>{state.집들이.family}</dd></div>
                            <div><dt>지역</dt><dd>{state.집들이.area} {state.집들이.area2==="null"?'':state.집들이.area2}</dd></div>
                            <div><dt>스타일</dt><dd>{state.집들이.style}</dd></div>
                        </dl>
                    </div>
